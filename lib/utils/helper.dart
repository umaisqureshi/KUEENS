...
class Helper{
  ...
  static Future<Marker> getMarkerImage(point, context) async {
    GeoPoint geoPoint = point.data()['location'];
    return Marker(
      onTap: (){
        Navigator.of(context).push(ShopInfoModel(point.id));
      },
        markerId: MarkerId(point.id),
      //  icon: await getMarkerIcon(point, Size(150,150), point.data()['shopData']['shopLogo']),
        icon: BitmapDescriptor.fromBytes(await getByteFromAsset('assets/images/marker.png',70)),
  ...
  static Future<BitmapDescriptor> getMarkerIcon(firebaseUser, Size size, shopLogo) async {
    ...
    final Paint shadowPaint = Paint()..color =Color(0xFF313c52).withOpacity(0.8);
