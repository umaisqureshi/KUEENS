...
class _EditPostState extends State<EditPost> {
  ...
  void initState() {
    ...
    pid = widget.map.param1['productId'];
  ...
  getCategories() {
    _selectedCategoryTitle = widget.map.param1['category'];
    _selectedCategoryValue = chooseCategories
        .where((element) => element.title == _selectedCategoryTitle)
        .first
        .value;
  }
  ...
  getData() {
    setState(() {
      titleController.text = widget.map.param1['title'];
      getCategories();
      initialPrice = widget.map.param1['price'];
/*      if (widget.map.containsKey('offer') && widget.map['offer'] != null) {
        offerController = 'Edit Offer';
        this.discountModel = DiscountModel(
            discount: widget.map['offer']['discount'],
            discountDesc: widget.map['offer']['discountDesc'],
            discountType: widget.map['offer']['discountType'],
            offerType: widget.map['offer']['offerType'],
            image: widget.map['offer']['image']);
      }*/
      descController.text = widget.map.param1['desc'];
      widget.map.param1['tags'].forEach((element) {
        selected.add(element.toString());
      });
      address = widget.map.param1['address'];
      geoPoint = widget.map.param1['location'];
      List.from(widget.map.param1['images']).forEach((element) {
        _imageAssets.add(ImageAssets(isFile: false, path: element));
      });
      List.from(widget.map.param1['videoUrl']).forEach((element) {
        videoList.add(VideoModel(isFile: false, path: element, thumbnail: null));
      });
    });
  }
                        ...
                        GestureDetector(
                          onTap: () async {
                            var result = await Navigator.of(context)
                                .push(CreatePostFragment(discountModel:discountModel==null?discountModel:null));
                            setState(() {
                                  ...
                                  if (videoFile != null) {
                                    ...
                                    setState(() {
                                      videoList.add(VideoModel(
                                          path: videoResult,
                                          thumbnail: uint8list, isFile: true));
                                    });
                                  }
                                },
                                child: Padding(
    ...
    return Padding(
      ...
      child: ListView(
        ...
        children: List.generate(_imageAssets.length, (index) {
          ImageAssets asset = _imageAssets[index];
          return Padding(
          ...
          return Padding(
            ...
            child: Stack(
              children: [
                ClipRRect(
                    borderRadius: BorderRadius.circular(10),
                    child: videoList[index].isFile?Image.memory(videoList[index].thumbnail):Container()),
                Positioned(
                  top: 1,
                  right: 1,
                  child: InkWell(
 ...
 Future uploadImage() async {
    for (int i = 0; i < _imageAssets.length; i++) {
      if (_imageAssets[i].isFile) {
        var reference = FirebaseStorage.instance
            .ref()
            .child('${DateTime.now().millisecondsSinceEpoch}.jpg');
        final filePath = File(_imageAssets[i].path).absolute.path;
        ...
        var url = (await (await uploadTask.onComplete).ref.getDownloadURL());
          _imageAssets[i] = ImageAssets(path: url, isFile: false);
      }
    }
 ...
 Future dataUpload() async {
    ...
    await FirebaseCredentials().db.collection('OfflinePost').doc(widget.map.param2).update({
      ...
      geoFirestore
          .setLocation(widget.map.param2, GeoPoint(geoPoint.latitude, geoPoint.longitude))
          .then((v) async {
        await callOnFcmApiSendPushNotifications(
            token, widget.map.param2, FirebaseCredentials().auth.currentUser.uid);
        showDialog(
          context: context,
  ...
  Future<void> loadAssets() async {
    ...
    try {
      ...
      for (Asset asset in resultList) {
        ...
        _imageAssets.add(ImageAssets(path: filePath, isFile: true));
      }
      setState(() {});
  ...
  Future uploadVideo() async {
    for(int i =0; i< videoList.length; i++){
     if(videoList[i].isFile){
       ...
       StorageUploadTask uploadTask = reference.putFile(File(videoList[i].path));
       var url =  (await (await uploadTask.onComplete).ref.getDownloadURL());
       videoList[i] = VideoModel(isFile: false, thumbnail: null, path: url);
     }
    }
    videoList.forEach((element) {
      videoUrl.add(element.path);
    });
  }
...
class ImageAssets {
  ...
  ImageAssets({this.path, this.isFile});
}
