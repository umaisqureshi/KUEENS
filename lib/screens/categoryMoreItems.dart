...
class CategoryMoreItems extends StatefulWidget {
  final String category;

  CategoryMoreItems({this.category});

  @override
  _CategoryMoreItemsState createState() => _CategoryMoreItemsState();
}

class _CategoryMoreItemsState extends State<CategoryMoreItems> {
  @override
        ...
        appBar: AppBar(
          ...
          leading: InkWell(
              onTap: () => Navigator.of(context)
                  .pushReplacementNamed('/home', arguments: widget.category),
              child: Icon(
                Icons.arrow_back,
...
class _WeeklyItemsState extends State<WeeklyItems> {
  ...
  void initState() {
    ...
    FirebaseCredentials()
        ...
        .collection('Fav')
        .doc(widget.documentSnapshot.data()['productId'])
        .get()
        .then((value) {
      if (value.exists) {
        setState(() {
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        image: NetworkImage(
                            widget.documentSnapshot.data().containsKey('offer')?widget.documentSnapshot.data()['offer']['image']:widget.documentSnapshot.data()['images'][0],
                        ),
                        fit: BoxFit.cover,
                      ),
                    ),
